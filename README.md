...
## Web Automation Builder 使用说明

Web Automation Builder 被动录制用户在浏览器中的操作。打开浏览器后用户自由操作，系统通过 CDP + DOM 事件注入自动记录点击、输入、导航等操作和 API 调用。录制完成后 LLM 分析生成结构化工作流。支持参数化重放、生成独立 Skill、导出 Playwright 脚本。

**前置依赖**：需要 [Playwright Skill](./skills/playwright/) 已安装。
...
```bash
# 1. 开始被动录制（启动浏览器 + 注入 DOM 监听 + 启动网络监听）
node skills/web-automation-builder/tool.js record '{"name":"部署后端代码"}'
# → 浏览器打开，用户自由操作，系统被动记录
...
# → 返回 rawEvents（DOM 事件 + API 请求），由 LLM 分析

# 4. 保存 LLM 分析后的结构化工作流
node skills/web-automation-builder/tool.js save '{"id":"wf-xxx","workflow":{...}}'
...
# 7. 导出 Playwright 脚本
node skills/web-automation-builder/tool.js export '{"id":"wf-xxx","output":"./deploy.js"}'
```
...
| 命令 | 说明 |
|------|------|
| `record` | 开始被动录制（启动浏览器 + DOM 监听 + 网络监听） |
| `stop` | 停止录制，返回原始录制数据（rawEvents） |
| `save` | 保存 LLM 分析后的结构化工作流 |
| `status` | 查看录制状态（已收集的事件数量） |
| `list` | 列出所有工作流 |
| `show` | 查看工作流详情 |
| `delete` | 删除工作流 |
| `replay` | 重放工作流（支持参数注入） |
| `generate` | 生成独立 Skill 目录 |
| `export` | 导出为 Playwright JS 脚本 |
| `install` | 安装到指定目录 |
...
Agent:
  1. 调用 record → 启动浏览器 + 注入监听
  2. 弹出 agent-interact wait 弹框 → "浏览器已打开，操作完成后点击确认"
  3. 等待用户操作...

用户: （在浏览器中自由操作：登录、点击、填表...）
用户: 操作完成了

Agent:
  1. 调用 stop → 获取 rawEvents（12 个 DOM 事件 + 5 个 API 请求）
  2. LLM 分析 → 生成结构化工作流，自动参数化用户名/密码/分支名
  3. 调用 save → 保存工作流
  4. 报告结果 → "录制完成！要生成 Skill 吗？"

用户: 生成 Skill
Agent: → 调用 generate，安装到 ~/.cursor/skills/deploy-staging

--- 下次新会话 ---
用户: 部署 release/2.3 到 staging
Agent: （识别到已安装的 Skill）→ 直接调用，注入 branch=release/2.3
```

### 触发词

- **录制**：录制浏览器操作、开始录制、帮我录制
- **重放**：重放工作流、再执行一次、用新参数执行
- **生成**：生成 Skill、做成 Skill、以后直接用
- **导出**：导出脚本、导出为 JS
- **管理**：查看录制、工作流列表
...
First Principles（第一性原理）将问题拆解到不可置疑的基础事实（物理定律 / 逻辑必然 / 实证数据），系统挑战每一个假设，再从地基向上重建解决方案。
...
- **复杂度路由**：T1 直接回答 → T4 完整分析 + 替代方案

### 适用场景

| 场景 | 触发信号 |
|------|---------|
| 🏗️ 架构决策 | "为什么用这个架构"、"重新设计" |
| 💰 成本质疑 | "为什么这么贵"、"成本能不能降" |
| 🔧 根因分析 | "为什么反复出现"、"根本原因" |
| 📋 产品定义 | "用户真正需要什么" |
| 🔬 技术选型 | "用 A 还是 B" |
...
```
Phase 1 拆解 → Phase 2 地基检验 → Phase 3 重建
                                         │
              ← 发现新假设时回溯 ←────────┘
```
...
AI (T3 架构决策):
  Phase 1: 拆解假设（团队 DevOps 能力、服务边界、数据一致性）
  Phase 2: 标记"单体已成为瓶颈"为伪约束
  Phase 3: 推荐 Modular Monolith → 验证边界后再拆分
```
...
| Skill | 定位 | 输出 |
|-------|------|------|
| **socratic** | 清理地基——追问和挑战 | 更清晰的定义 |
| **first-principles** | 从地基重建——拆解到基岩再构建 | 具体方案 |

推荐顺序：`/socratic` → `/first-principles`
