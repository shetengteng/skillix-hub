...
| [web-automation-builder](./skills/web-automation-builder/) | 录制浏览器操作序列，保存为可重放的参数化工作流，支持生成独立 Skill 和导出 Playwright 脚本 |
...
Web Automation Builder 录制浏览器操作序列，保存为可重放的参数化工作流。支持生成独立 Skill（安装到 ~/.cursor/skills/）和导出标准 Playwright 脚本。
...
node skills/web-automation-builder/tool.js exec '{"command":"click","args":{"ref":"e15","element":"登录"}}'
...
| `stop` | 停止录制并保存工作流 |
| `status` | 查看录制状态 |
| `exec` | 代理执行 Playwright 命令（录制模式下自动记录） |
| `list` | 列出所有工作流 |
| `show` | 查看工作流详情 |
| `delete` | 删除工作流 |
| `replay` | 重放工作流（支持参数注入） |
| `analyze` | 分析工作流，输出参数化建议 |
| `generate` | 生成独立 Skill 目录 |
| `export` | 导出为 Playwright JS 脚本 |
| `install` | 安装到指定目录（复制源码） |
| `update` | 更新已安装的 Skill（保留工作流数据） |

### 自然语言使用示例

用户通过自然语言与 Agent 交互，Agent 自动调用录制/重放/生成命令：

```
用户: 帮我录制一下登录公司后台的操作
Agent: 好的，开始录制。→ 自动执行 record + exec 代理模式

用户: 把用户名和密码参数化
Agent: 已参数化！→ 自动执行 analyze，将输入值替换为 {{username}} {{password}}

用户: 用 lisi 的账号登录一下
Agent: → 自动执行 replay，注入 lisi 的参数

用户: 把这个做成 Skill，以后直接用
Agent: → 自动执行 generate，安装到 ~/.cursor/skills/

--- 下次新会话 ---
用户: 用 wangwu 登录公司后台
Agent: （识别到已安装的 Skill）→ 直接调用生成的 Skill
```
