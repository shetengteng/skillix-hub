...
**规则：**
1. **Layer 1 是入口**。AI 收到知识相关问题时，**先读 INDEX.md 的 Topic Registry**。Registry 包含每个 topic 的 summary/tags/answers，足够做首轮召回。
2. **Layer 2 做精筛**。仅对 Layer 1 召回的候选 topic，读取 frontmatter 确认相关性。
3. **Layer 3 按需读取**。只读与问题相关的 1-2 篇文章正文，优先读 high/medium 覆盖度的章节。
4. **Layer 4 最后手段**。仅在 low 覆盖度章节不足以回答时才读原始文件。
...
6. **Mermaid 图表**：架构类、流程类、状态机类 topic 推荐包含 mermaid 图表；概念类和配置类不强制
