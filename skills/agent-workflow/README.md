...
> **当前版本**：v1.5.3（已实现，Python 3.10+）
> **完整 Agent 规约**：`SKILL.md`
> **在线文档**：[docs/index.html](../../docs/index.html) 中 `agent-workflow` 卡片
> **设计与验收**：`design/agent-workflow/` 下 5 份文档

跨 IDE / 跨 LLM 的声明式工作流引擎，以 CLI 状态机形态运行。Claude Code / Cursor / Codex / OpenCode 通用，纯 Python 3.10+ 实现，零运行时网络依赖。
...
+------------------+              +------------------+    Prompt    +------------------+
                                          ^
                                          v
                          disk: .agent-workflow/runs/<run_id>/
                          (state.json, events.ndjson, audit.log, outputs/, workflow.yaml)
```
...
2. [何时该用 / 不该用](#何时该用--不该用)
3. [安装](#安装)
4. [5 分钟跑通第一个 workflow](#5-分钟跑通第一个-workflow)
5. [Workflow YAML 完整写法](#workflow-yaml-完整写法)
6. [4 种节点类型字段速查](#4-种节点类型字段速查)
7. [10 个 CLI 命令](#10-个-cli-命令)
8. [集成到 IDE Agent：caller 主循环](#集成到-ide-agent-caller-主循环)
9. [接外部 LLM CLI（真实跑通的写法）](#接外部-llm-cli真实跑通的写法)
10. [view 命令：浏览器可视化 run](#view-命令浏览器可视化-run)
11. [长链稳定性：chain_timeout & stall watchdog](#长链稳定性chain_timeout--stall-watchdog)
12. [secrets 与 ENV](#secrets-与-env)
13. [错误处理：retryable 判定](#错误处理retryable-判定)
14. [调试 recipes](#调试-recipes)
15. [测试](#测试)
16. [目录结构](#目录结构)
17. [FAQ](#faq)
...
| `view` | 生成自包含 HTML，浏览器可视化查看 run（v1.5.3） |
...
实测：`claude -p` 平均回包 8–12s，`codex exec` 类似。如需全局生效：写入 `~/.agent-workflow/config.yaml` 的 `executors:` 段，所有 workflow 都会继承。
...
| `EXECUTOR_STALLED` | ❌ | 终态，检查外部 CLI 状态 |
...
每个 run 在 `.agent-workflow/runs/<run_id>/` 下有完整审计目录：
...
# 4) 看 audit.log 时间线
less .agent-workflow/runs/wf-.../audit.log
```
...
A: 所有状态在 `.agent-workflow/runs/<run_id>/state.json`。重启后用 `list '{"status":["waiting_user","awaiting_agent"]}'` 找未完成的 run，用 `status` 拿 `last_payload` 重建上下文，直接 `resume` / `advance` 即可。
...
**Q: 怎么写新的 workflow 模板？**
A: 把 YAML 放进 `skills/agent-workflow/examples/<name>.yaml`，CLI 自动在 `create '{"action":"list_templates"}'` 里识别到。
