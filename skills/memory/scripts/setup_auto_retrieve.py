...
description: Memory Skill 自动记忆规则 - 在对话开始时自动检索相关历史记忆，在对话结束时智能保存有价值的内容
globs:
alwaysApply: true
---
...
name: memory-skill
description: Memory Skill 自动记忆规则 - 在对话开始时自动检索相关历史记忆（当用户问题包含延续性词汇如上次、之前、昨天、继续，偏好相关词汇如我喜欢、我习惯，或项目相关词汇如这个项目、我们的时触发），在对话结束时智能保存有价值的内容（重要决策、用户偏好、项目配置等）
