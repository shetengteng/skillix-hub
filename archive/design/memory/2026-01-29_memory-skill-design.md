...
**核心挑战**：AI 无法可靠地检测到"对话结束"时刻，因为 Cursor 可能在任何时候被关闭，不会给 AI 执行保存操作的机会。

#### 6.3.2 解决方案：三层保障机制

借鉴 Behavior Prediction Skill 的设计，采用**三层保障机制**确保记忆数据完整性：
...
**策略**：不等到对话结束，而是在每次有价值的对话后立即保存。

```
用户提问 → AI 回答 → 判断是否值得保存 → 立即执行 save_memory.py
```
...
**注意**：此层是可选的，因为 Cursor 可能在任何时候被关闭，不保证能执行。

##### 第三层：下次会话检查（兜底）

**策略**：在每次新会话开始时，检查上次会话是否有未保存的内容。

```
新会话开始 → 检查上次会话状态 → 补充遗漏的记忆 → 继续正常对话
```

**实现方式**：
1. 在 Always Applied Rule 中添加会话开始检查
2. 检查最后一次保存时间
3. 如果发现异常（如上次会话没有正常结束），触发补充保存

#### 6.3.3 实施建议

1. **优先实现第一层**：实时保存是最可靠的方案
2. **第二层作为增强**：如果能检测到结束信号，进行汇总
3. **第三层作为兜底**：确保数据完整性

#### 6.3.4 与 Behavior Prediction Skill 的对比

| 特性 | Memory Skill | Behavior Prediction Skill |
|------|--------------|---------------------------|
| 数据类型 | 对话记忆 | 行为序列 |
| 实时保存 | 有价值的对话后保存 | 每次工具调用后记录 |
| 会话结束 | 汇总保存 | 更新转移矩阵 |
| 下次检查 | 补充遗漏记忆 | 重新计算矩阵 |

两个 Skill 面临相同的挑战，采用相同的三层解决方案，确保数据完整性。
