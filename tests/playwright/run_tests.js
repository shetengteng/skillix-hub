...
const SKILL_DIR = path.resolve(THIS_DIR, '../../skills/playwright');
...
async function runTest(testFile) {
  ...
  try {
    const output = execSync(`node "${testFile}"`, {
      cwd: SKILL_DIR,
      timeout: 120000,
      env: { ...process.env, SKILL_DIR, PLAYWRIGHT_SKILL_HEADLESS: 'true' },
      encoding: 'utf-8',
...
async function main() {
  ...
  console.log(`\n${'='.repeat(60)}`);
  console.log(`  ${results.length} tests | ${passed} passed | ${failed} failed | ${(elapsed / 1000).toFixed(1)}s`);
  console.log(`${'='.repeat(60)}`);
