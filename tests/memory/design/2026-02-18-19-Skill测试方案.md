...
| 目录 | 职责 | 谁使用 |
|------|------|--------|
| `skills/memory/` | **源码** — 开发、修改、测试 | 开发者 |
| `.cursor/skills/memory/` | **安装副本** — Cursor Hooks 实际执行 | Cursor IDE |
...
| 文件 | 源码 | 安装版 | 差异原因 |
|------|------|--------|----------|
| `SKILL.md` | `{{SCRIPT_PATH}}/save_fact.py` | `.cursor/skills/memory/scripts/save_fact.py` | Agent 提示用 |
...
- 从源码运行 → `SCRIPT_DIR = .../skills/memory/scripts/`
- 从安装版运行 → `SCRIPT_DIR = .../.cursor/skills/memory/scripts/`
...
```
    ...
    └──────┬───────┘
           ▼
    ┌──────────────┐
    │  运行测试     │  python3 skills/memory/tests/run_tests.py
    └──────┬───────┘
           ▼
    ┌──────────────┐
    │  全部通过？   │
...
```bash
# === 步骤 1：修改源码 ===
vim skills/memory/scripts/load_memory.py

# === 步骤 2：运行全量测试（约 140s） ===
python3 skills/memory/tests/run_tests.py
# 报告输出: 设计/2026-02-18-15-测试报告.md

# === 步骤 3：确认全部通过后安装 ===
python3 skills/memory/scripts/init.py --skip-model

# === 步骤 4：验证安装一致性 ===
diff -rq skills/memory/ .cursor/skills/memory/ \
...
```
python3 /path/to/skills/memory/scripts/save_fact.py --content "..."
```

而安装后运行时输出的是：

```
python3 /path/to/.cursor/skills/memory/scripts/save_fact.py --content "..."
```
...
```bash
# 克隆源码后，在目标项目中运行：
python3 /path/to/clawd.bot/skills/memory/scripts/init.py \
  --project-path /path/to/target-project

# 或全局安装：
python3 /path/to/clawd.bot/skills/memory/scripts/init.py --global
```

### init.py 执行的操作

1. 创建 `.cursor/skills/memory/` — 复制 Python 代码
...
6. 下载嵌入模型到 `~/.memory/models/`（全局缓存，不重复下载）
...
echo '{"workspace_roots":["/path/to/target-project"]}' | \
  python3 /path/to/target-project/.cursor/skills/memory/scripts/load_memory.py
```
