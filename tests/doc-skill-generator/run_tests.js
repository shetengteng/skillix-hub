...
const SKILL_DIR = path.resolve(THIS_DIR, '../../skills/doc-skill-generator');
...
function buildMarkdown(results, elapsed) {
  ...
  const lines = [
    '# Doc Skill Generator 测试报告', '',
    `> 时间: ${new Date().toISOString().replace('T', ' ').slice(0, 19)}`,
    `> 结果: ${status}`, `> 耗时: ${(elapsed / 1000).toFixed(3)}s`, '',
    '## 汇总', '', '| 指标 | 数值 |', '|---|---:|',
    `| 总用例 | ${results.length} |`, `| 通过 | ${passedCount} |`, `| 失败 | ${failedCount} |`, '',
  ];
  for (const cat of SUB_DIRS) {
    ...
    for (const r of catResults) lines.push(`| \`${r.name}\` | ${r.status} | ${(r.elapsed / 1000).toFixed(2)}s |`);
    lines.push('');
  }
  const failures = results.filter(r => r.status === 'FAIL');
  if (failures.length) {
    lines.push('## 失败详情', '');
    for (const r of failures) lines.push(`### FAIL: \`${r.name}\``, '', '```text', r.output.trim(), '```', '');
  }
  return lines.join('\n') + '\n';
}
...
async function main() {
  const start = Date.now();
  const allTests = [];
  for (const sub of SUB_DIRS) allTests.push(...findTestFiles(sub));
  if (allTests.length === 0) { console.log('No test files found.'); return 0; }
  ...
  const passedCount = results.filter(r => r.status === 'PASS').length;
  const failedCount = results.filter(r => r.status === 'FAIL').length;
  console.log(`\n${'='.repeat(60)}\n  ${results.length} tests | ${passedCount} passed | ${failedCount} failed | ${(elapsed / 1000).toFixed(1)}s\n${'='.repeat(60)}`);
