...
交互弹框的触发**不应由用户指定类型**，而应由 LLM 在任务执行过程中**自主判断**何时需要用户介入，并选择最合适的交互类型。

用户不需要知道存在哪些弹框类型，也不需要说"弹个 confirm 框"。LLM 根据**任务上下文**做出决策。
...
| 场景 | 决策类型 | 说明 |
|------|----------|------|
| 存在多个可行方案，无法自动判断用户意图 | confirm | 如：检测到 3 个数据库，不确定用哪个 |
| 即将执行不可逆/高风险操作 | approval | 如：删除数据、修改生产配置 |
| 需要用户提供结构化信息才能继续 | form | 如：缺少数据库连接参数 |
| 需要等待用户完成外部操作 | wait | 如：需要用户在另一个系统中审批 |
| 任务完成，需要向用户展示数据分析结果 | chart | 如：性能分析完成，展示趋势图 |
| 任务状态变更，需要通知用户 | notification | 如：部署完成、构建失败 |
| 长时间任务进行中，需要展示进度 | progress | 如：多步骤部署流程 |
...
LLM 执行任务
  ...
  │    │   └─ 通知状态？ → notification
  │    │
  │    ├─ 构造请求参数（title、message、options 等）
  │    ├─ 调用 tool.js dialog '<JSON>'
  │    ├─ 等待用户响应
  │    └─ 根据响应继续执行
  │
  └─ 无需用户介入 → 继续自动执行
...
1. **approval** — 涉及不可逆操作时，安全优先
2. **form** — 缺少必要信息时，信息收集优先
3. **confirm** — 存在多个选项时，让用户决策
4. **wait** — 需要外部操作时
5. **notification** — 仅通知，不阻塞流程
6. **chart / progress** — 信息展示类

### 7.5 反模式（不应出现的情况）

| 反模式 | 说明 |
|--------|------|
| 用户说"弹个 confirm 框" | 用户不应知道框的类型名称 |
| LLM 每一步都弹框确认 | 过度交互，应只在关键节点介入 |
| 能自动判断却要求用户选择 | 如只有一个选项时不应弹 confirm |
| 用 confirm 代替 approval | 高风险操作必须用 approval，明确 approve/reject 语义 |
| 用 confirm 收集信息 | 需要多字段输入时应用 form |
