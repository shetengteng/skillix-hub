...
**核心原则**：让用户通过自然语言安装 Skill，不需要理解技术原理，降低心智负担。
...
在 `SKILL.md` 文件中添加 `## 安装后提示` 部分，AI 安装完成后会读取并展示给用户：
...
运行 `启用自动行为记录` 开启自动记录功能，这样 AI 会自动记录你的每次操作并学习你的行为模式。

### 快速开始

| 命令 | 说明 |
|------|------|
| `启用自动行为记录` | 开启自动记录功能（推荐） |
| `查看我的行为模式` | 查看已学习到的行为模式 |
| `预测下一步` | 手动触发预测建议 |
| `查看行为统计` | 查看统计数据概览 |

### 首次使用提示

- 🆕 首次安装时没有历史数据，需要使用一段时间积累行为记录
- 📊 至少需要 3 次相同的行为序列才会开始预测
...
为了让 Skill 能够自动记录用户行为，需要创建 Always Applied Rule。

**脚本**：`scripts/setup_auto_record.py`
...
**规则文件位置**：
- Cursor: `.cursor/rules/behavior-prediction-auto-record.mdc`
- Claude: `.claude/rules/behavior-prediction-auto-record.md`
- 通用: `.ai/rules/behavior-prediction-auto-record.md`

### 15.5 用户命令

| 命令 | 说明 |
|------|------|
| `启用自动行为记录` | 创建规则文件，开启自动记录 |
| `禁用自动行为记录` | 删除规则文件，关闭自动记录 |
| `检查自动行为记录状态` | 查看是否已启用 |
| `更新自动行为记录规则` | 更新规则到最新版本 |

### 15.6 用户体验流程

```
用户: 帮我安装 behavior-prediction skill
...
AI: ✅ 已启用自动行为记录！
    规则文件已创建: .cursor/rules/behavior-prediction-auto-record.mdc
    
    现在 AI 会自动记录你的每次操作，学习你的行为模式。
    使用一段时间后，会在适当时机提供预测建议。
```
