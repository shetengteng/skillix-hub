...
写入格式：
```bash
python3 .cursor/skills/memory-skill/scripts/save_fact.py --content "事实内容" --type W --entities "实体" --confidence 0.9 --session "会话ID"
```
...
写入格式：
```bash
python3 .cursor/skills/memory-skill/scripts/save_summary.py --topic "主题" --summary "摘要" --decisions "决策" --todos "待办" --session "会话ID"
```
...
```bash
python3 ~/.cursor/skills/memory-skill/scripts/search_memory.py "查询内容" --max-results 10
```
...
| 文件 | 路径 | 加载方式 |
|------|------|----------|
| SKILL.md | `~/.cursor/skills/memory-skill/SKILL.md` | Cursor 自动加载（当触发条件匹配时） |
