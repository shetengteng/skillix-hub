...
| 模型 | GLOBAL_MODEL_CACHE | ~/.memory-skill/models | 否 |
...
3. 全局配置        ~/.memory-skill/config.json
...
| `cache_dir` | string | "~/.memory-skill/models" | 模型缓存目录 |
...
加载代码默认值（_DEFAULTS 字典）
    ↓
检查全局配置 ~/.memory-skill/config.json
    ↓ 存在则合并（deep merge）
...
class Config:
    ...
    def __init__(self, project_path=None):
        ...
        # 层级 1：全局配置
        global_cfg = self._load_json("~/.memory-skill/config.json")
        if global_cfg:
