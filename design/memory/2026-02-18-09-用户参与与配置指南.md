...
```bash
# 将 memory 复制到 Cursor skills 目录
cp -r memory ~/.cursor/skills/memory
```
...
```bash
python3 ~/.cursor/skills/memory/scripts/init.py [--global]
```
...
| 6. 下载嵌入模型 | 预下载到 `~/.cursor/skills/memory/models/` | 默认 BGE-small-zh-v1.5（开发），可选 Qwen3-Embedding-0.6B（生产） |
...
def download_model(model_name):
    ...
    print(f"  ✓ {model_name} 下载完成（缓存于 ~/.memory/models/）")
```
...
python3 ~/.cursor/skills/memory/scripts/init.py

# 验证通过后，升级到生产模型
python3 ~/.cursor/skills/memory/scripts/init.py --production

# 仅配置文件（已有依赖和模型时）
python3 ~/.cursor/skills/memory/scripts/init.py --skip-model
```

**hooks.json 合并策略**：如果 `hooks.json` 已存在（用户有其他 Hook），init.py 会读取现有配置并合并 memory 的 Hook，不会覆盖用户已有的配置。
