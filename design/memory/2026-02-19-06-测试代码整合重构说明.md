...
> 日期：2026-02-19  
> 范围：将 `skills/memory-skill/tests` 测试体系迁移并整合到 `tests/standalone-memory-skill`
...
2. 将测试统一收口到 `tests/standalone-memory-skill`，形成单一执行入口与统一报告输出
...
- `skills/memory-skill/tests/*`（legacy 测试）
- `tests/standalone-memory-skill/*`（外部独立测试）

## 2.2 迁移后

```text
tests/standalone-memory-skill/
...
`skills/memory-skill/tests` 目录已去除。
...
文件：`tests/standalone-memory-skill/run_tests.py`
...
文件：`tests/standalone-memory-skill/test_detail_*.py`
...
1. 目录整合已完成：所有测试 `.py` 均在 `tests/standalone-memory-skill` 同一层
...
1. 测试入口单一化：后续只需维护 `tests/standalone-memory-skill/run_tests.py`
